...
import 'package:admin_app_flutter/model/admin_model.dart';
...
class _ChangeProfileState extends State<ChangeProfile> with Helpers {
  ...
  void initState() {
    ...
    _admin = FbAuthController().user;
    _username = TextEditingController(text: _admin.displayName ?? 'No Name');
    super.initState();
  }
  ...
  Future<bool> updateDisplayName() async {
    ...
    if (status) {
      bool createUser = await FbStoreController()
          .createUser(userModel: userModel,collectionName: 'Admins');
      if (createUser) {
